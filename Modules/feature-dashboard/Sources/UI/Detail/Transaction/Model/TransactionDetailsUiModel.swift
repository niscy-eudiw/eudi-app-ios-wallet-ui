...
extension TransactionLogDomain {
  ...
  private var cardParty: (label: LocalizableStringKey, subtitles: [String], details: [[TransactionDetailsFieldUi]]) {
    switch self {
    case .presentation(let log):
      ...
      return (.transactionDetailsRelyingPartyLabel, log.party.subtitles, groups)
    case .credentialIssuance(let log):
      return (.transactionDetailsIssuerLabel, log.details.issuer.subtitles, [TransactionDetailsFieldUi.issuerFields(log.details)].filter { !$0.isEmpty })
    ...
    case .credentialDeletion(let log):
      ...
      return (.transactionDetailsIssuerLabel, log.issuer.subtitles, [fields].filter { !$0.isEmpty })
    case .signingSealing(let log):
      ...
      return (.transactionDetailsSigningServiceLabel, log.service.subtitles, [details].filter { !$0.isEmpty })
    case .dataDeletionRequest(let log):
      return (.transactionDetailsRelyingPartyLabel, log.party.subtitles, [])
    case .dpaReport(let log):
      return (.transactionDetailsAuthorityLabel, [log.dpaCountry].compactMap { $0 }, [])
    }
  }
